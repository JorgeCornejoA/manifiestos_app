...
class SupabaseService {
  ...
  Future<void> syncPendingManifests() async {
    ...
    try {
      print(
          '🌐 Iniciando sincronización de ${pending.length} manifiestos offline...');
      final currentEmp = await getCurrentEmployee();
      final usuarioSincronizador =
          currentEmp?.name ?? 'Sincronización Automática';
      ...
      for (int i = pending.length - 1; i >= 0; i--) {
        try {
          ...
          final evidencePhotosBytes =
              (item['evidencePhotosBytes'] as List<dynamic>?)
                  ?.cast<Uint8List>();
          ...
          if (evidencePhotosBytes != null && evidencePhotosBytes.isNotEmpty) {
            for (var bytes in evidencePhotosBytes) {
              ...
              final url =
                  _supabase.storage.from('manifests').getPublicUrl(path);
          ...
          if (embarcoFirmaBytes != null && embarcoFirmaBytes.isNotEmpty) {
            ...
            await _supabase.storage.from('manifests').uploadBinary(
                path, embarcoFirmaBytes,
                fileOptions: const FileOptions(upsert: true));
            embarcoFirmaUrl =
                '${_supabase.storage.from('manifests').getPublicUrl(path)}?t=$timestamp';
          }
          if (recibioFirmaBytes != null && recibioFirmaBytes.isNotEmpty) {
            final path = 'signatures/$manifestId/recibio_firma.png';
            await _supabase.storage.from('manifests').uploadBinary(
                path, recibioFirmaBytes,
                fileOptions: const FileOptions(upsert: true));
            recibioFirmaUrl =
                '${_supabase.storage.from('manifests').getPublicUrl(path)}?t=$timestamp';
          }
          ...
          final savedRecord = await _supabase
              .from('manifests')
              .upsert(rawData)
              .select()
              .single();
          ...
          final pdfBytes = await PdfGenerator.generatePdfBytes(savedManifest,
              nombreUsuario: usuarioSincronizador);
          ...
          final pdfUrl =
              '${_supabase.storage.from('manifests').getPublicUrl(pdfPath)}?t=$timestamp';

          // 6. Actualizar BD
          await _supabase
              .from('manifests')
              .update({'pdf_url': pdfUrl}).eq('id', savedManifest.id!);
  ...
  Future<ManifestData?> saveManifest(ManifestData data) async {
    try {
      ...
      final timestamp =
          DateTime.now().millisecondsSinceEpoch; // Sello anti-caché
      ...
      if (data.evidencePhotosBytes != null &&
          data.evidencePhotosBytes!.isNotEmpty) {
      ...
      if (data.embarcoFirmaBytes != null &&
          data.embarcoFirmaBytes!.isNotEmpty) {
        final path = 'signatures/$manifestId/embarco_firma.png';
        await _supabase.storage.from('manifests').uploadBinary(
              path,
      ...
      if (data.recibioFirmaBytes != null &&
          data.recibioFirmaBytes!.isNotEmpty) {
        final path = 'signatures/$manifestId/recibio_firma.png';
        await _supabase.storage.from('manifests').uploadBinary(
              path,
  ...
  Future<void> updatePdfUrl(String manifestId, String pdfUrl) async {
    try {
      ...
      await _supabase
          .from('manifests')
          .update({'pdf_url': pdfUrl}).eq('id', manifestId);
    } catch (e) {
  ...
  Future<List<CompanyTrailer>> getCompanyTrailers() async {
    try {
      ...
      return LocalDbService.getData(LocalDbService.trailersBox)
          .map((e) => CompanyTrailer.fromMap(e))
          .toList();
    }
  }
    ...
    try {
      ...
      if (await _hasInternet()) {
        ...
        final localEmployees =
            LocalDbService.getData(LocalDbService.employeesBox);
  ...
  Future<List<Employee>> getEmployees() async {
    try {
      ...
      return LocalDbService.getData(LocalDbService.employeesBox)
  ...
  Future<void> saveEmployee(Employee employee, Uint8List? signatureBytes,
      {String? password}) async {
    String? signatureUrl = employee.signatureUrl;
    String targetEmployeeId = employee.id ?? '';
    if (signatureBytes != null && signatureBytes.isNotEmpty) {
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final path = 'employee_signatures/${_uuid.v4()}_$timestamp.png';
      await _supabase.storage.from('manifests').uploadBinary(
          path, signatureBytes,
          fileOptions: const FileOptions(upsert: true));
      signatureUrl = _supabase.storage.from('manifests').getPublicUrl(path);
    }
    if (employee.id == null && password != null && password.isNotEmpty) {
      if (employee.email == null || employee.email!.isEmpty) {
        throw Exception("El correo es obligatorio para crear un acceso.");
      }
      try {
        ...
        if (authRes.user == null) {
          throw Exception("No se pudo generar la credencial de acceso.");
        }
        targetEmployeeId = authRes.user!.id;
      } on AuthException catch (e) {
        if (e.message.contains("User already registered")) {
          throw Exception("Este correo ya pertenece a otro empleado.");
        }
        throw Exception("Error de seguridad: ${e.message}");
      }
    } else if (employee.id == null) {
      targetEmployeeId = _uuid.v4();
    }
// MAPEO CON LA NUEVA COLUMNA INCLUIDA
    final map = employee.toMap();
    map['signature_url'] = signatureUrl;
    if (employee.id == null) {
      map['id'] = targetEmployeeId;
  ...
  Future<List<Client>> getClients() async {
    try {
      ...
      return LocalDbService.getData(LocalDbService.clientsBox)
  ...
  Future<List<Operator>> getOperators() async {
    try {
      ...
      return LocalDbService.getData(LocalDbService.operatorsBox)
          .map((map) => Operator.fromMap(map))
          .toList();
    }
  }
  ...
  Future<List<Map<String, dynamic>>> searchProducts(String query) async {
    try {
      final response = await _supabase
          .from('t_producto')
          .select(
              'c_codigo_pro, v_nombre_pro, c_codigo_tam, t_tamano(v_nombre_tam)')
